#!/usr/bin/env python3
"""
NPU Mel Spectrogram Processor - Batch Processing Final (30 frames)

This is the PRODUCTION BATCH PROCESSOR optimized for 30-frame chunks,
designed to work with the batch MLIR kernel (30 frames per NPU call).

Key Optimizations:
- Pre-allocated 24KB input buffer (30 frames × 800 bytes)
- Pre-allocated 2400B output buffer (30 frames × 80 bytes)
- Single sync per batch (not per frame)
- Processes audio in 30-frame chunks
- Handles partial batches (last batch may be < 30)
- Reuses XRT buffer objects for zero allocation overhead

Performance Target:
- Single-frame: 3.7ms/frame (20-30x realtime)
- Batch-20: 0.18ms/frame (1200-1500x realtime)
- Batch-30: 0.12ms/frame (1800-2250x realtime) ← 1.5x improvement
- Expected speedup: 1.5x faster mel preprocessing (45x → 67x realtime)

Hardware: AMD Phoenix NPU (XDNA1)
Kernel: mel_batch30.xclbin (30-frame batch processing)
Author: Magic Unicorn Unconventional Technology & Stuff Inc.
Date: November 2, 2025 (Upgraded from batch-20)
"""
...
class NPUMelProcessorBatch:
    """
    Production batch NPU mel spectrogram processor (30 frames per call).

    This class processes audio in 30-frame batches using a custom MLIR kernel
    that processes all 30 frames in a single NPU call, reducing XRT overhead
    by 30x compared to single-frame processing.

    Architecture:
    - Input buffer: 24KB (30 frames × 400 samples × 2 bytes int16)
    - Output buffer: 2400B (30 frames × 80 mel bins × 1 byte int8)
    ...
    # Constants
    BATCH_SIZE = 30  # Fixed batch size matching MLIR kernel (upgraded from 20 for 1.5x performance)
    ...
    def __init__(
        ...
        if xclbin_path is None:
            default_path = Path(__file__).parent / "mel_kernels" / "build_batch30" / "mel_batch30.xclbin"
        ...
        xclbin_name = Path(self.xclbin_path).stem  # e.g., "mel_batch30" or "mel_batch100"
        insts_name = xclbin_name.replace("mel_", "insts_") + ".bin"  # e.g., "insts_batch30.bin"
        ...
        self.instr_bo = None
        self.input_bo = None   # 24KB buffer (30 frames)
        self.output_bo = None  # 2400B buffer (30 outputs)
        self.insts_bin = None
        self.n_insts = 0

        # Buffer sizes
        self.input_buffer_size = self.BATCH_SIZE * self.FRAME_SIZE * 2  # 24,000 bytes (batch-30)
        self.output_buffer_size = self.BATCH_SIZE * self.N_MELS         # 2400 bytes (batch-30)
    ...
    def _initialize_npu(self) -> bool:
        ...
        try:
            ...
            if self.verbose:
                logger.info("="*70)
                logger.info("Initializing AMD Phoenix NPU (Batch-30 Mode)")
                logger.info("="*70)
            ...
            if self.verbose:
                logger.info(f"Kernel: MLIR_AIE (batch-30 mel spectrogram)")
            ...
            # Pre-allocate input buffer (24KB for 30 frames)
            ...
            # Pre-allocate output buffer (2400B for 30 outputs)
    ...
    def _process_batch_npu(self, frames: np.ndarray, batch_size: int) -> np.ndarray:
        """
        Process a batch of frames on NPU (up to 30 frames).
        ...
        Args:
            ...
            batch_size: Actual number of frames in this batch (≤30)
    ...
    def process(self, audio: np.ndarray) -> np.ndarray:
        ...
        2. Processes frames in batches of 30 using NPU
        3. Handles partial batches (last batch may be < 30)
        ...
        # Pre-allocate frame buffer (30 frames max)
...
if __name__ == "__main__":
    # Quick test
    logger.info("NPU Mel Processor Batch-30 - Quick Test")
    logger.info("="*70)

#!/bin/bash
# Compile MEL kernel with BATCH-20 optimization
# November 1, 2025

set -e  # Exit on error

echo "======================================================================"
echo "MEL Kernel Compilation - BATCH 20 Optimization"
echo "======================================================================"
echo
echo "Batch size: 20 frames per NPU invocation"
echo "Expected speedup: 12-17x vs single-frame processing"
echo "Memory usage: ~39.3 KB (61% of 64 KB AIE tile memory)"
echo
echo "======================================================================"
echo
...
BUILD_DIR=$WORK_DIR/build_batch20
mkdir -p $BUILD_DIR
cd $WORK_DIR
...
cp mel_fixed_v3_batch20.mlir $BUILD_DIR/
cp mel_fixed_combined.o $BUILD_DIR/
cd $BUILD_DIR
echo "✅ Files copied"
echo

echo "Step 3: Generate XCLBIN with aiecc.py (batch-20 version)..."
time /home/ucadmin/mlir-aie-fresh/mlir-aie/venv313/bin/aiecc.py \
    ...
    --npu-insts-name=insts_batch20.bin \
...
if [ -f "$BUILD_DIR/mel_batch20.xclbin" ]; then
    echo "Generated Files:"
    ls -lh $BUILD_DIR/mel_batch20.xclbin $BUILD_DIR/insts_batch20.bin
    echo

    echo "XCLBIN Information:"
    xclbinutil --input $BUILD_DIR/mel_batch20.xclbin --info | head -50
    echo

    echo "======================================================================"
    echo "✅ SUCCESS! Batch-20 kernel ready for NPU execution"
    echo "======================================================================"
    echo
    echo "Performance expectations:"
    echo "  - Batch size: 20 frames per call"
    echo "  - Expected speedup: 12-17x vs single-frame"
    echo "  - For 1h 44m audio: ~8-11 seconds (from 134s)"
    echo
    echo "Next step: Test with Python wrapper"
    echo "  python3 test_mel_batch20.py"
else

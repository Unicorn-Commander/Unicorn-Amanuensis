//===- matmul_bf16_vectorized.mlir ---------------------------------*- MLIR -*-===//
//
// Vectorized BF16 Matrix Multiplication for Whisper Encoder - XDNA1 Phoenix NPU
// Tile-based matmul for attention and FFN layers
//
// C = A * B
// Using BF16 inputs/outputs with FP32 accumulation via AIE2 vector intrinsics
//
// Fixed size: 64x64 matrices
// Input A: 64x64 bfloat16 = 8192 bytes
// Input B: 64x64 bfloat16 = 8192 bytes
// Output C: 64x64 bfloat16 = 8192 bytes
//
// Performance target: 30-200x speedup over scalar (49ms for 64x64)
//
//===----------------------------------------------------------------------===//

module @matmul_bf16_vectorized_npu {
    aie.device(npu1) {
        // Declare external vectorized MatMul kernel function
        // Takes three 64x64 BF16 matrices (as raw bytes)
        // C = A * B
        ...
        // Input A ObjectFIFO: 64x64 bfloat16 elements = 8192 bytes
        aie.objectfifo @of_input_A(%tile00, {%tile02}, 2 : i32) : !aie.objectfifo<memref<8192xi8>>

        // Input B ObjectFIFO: 64x64 bfloat16 elements = 8192 bytes
        aie.objectfifo @of_input_B(%tile00, {%tile02}, 2 : i32) : !aie.objectfifo<memref<8192xi8>>

        // Output C ObjectFIFO: 64x64 bfloat16 elements = 8192 bytes
        ...
        %core02 = aie.core(%tile02) {
            ...
            scf.for %iter = %c0 to %c_max step %c1 {
                ...
                // Call vectorized MatMul kernel: C = A * B

#!/usr/bin/env python3
"""
Test Parallel Softmax - 2 Tiles Processing Simultaneously
...
def main():
    print("=" * 70)
    print("Parallel Softmax BF16 - 2 Tiles Test - AMD Phoenix NPU")
    print("=" * 70)
    print()
    ...
    xclbin_path = "build_softmax_parallel/softmax_parallel.xclbin"
    ...
    try:
        ...
        # Prepare test data - 2 different frames
        print("Step 5: Preparing test data for 2 frames...")
        input_floats = []
        input_bf16 = []
        expected = []
        ...
        # Allocate 4 XRT buffers (2 input + 2 output)
        print("Step 6: Allocating 4 XRT buffers...")
        ...
        # Input buffers (2)
        bo_in0 = xrt.bo(device, buffer_size, xrt.bo.flags.host_only, kernel.group_id(3))
        bo_in1 = xrt.bo(device, buffer_size, xrt.bo.flags.host_only, kernel.group_id(4))
        
        # Output buffers (2)
        bo_out0 = xrt.bo(device, buffer_size, xrt.bo.flags.host_only, kernel.group_id(5))
        bo_out1 = xrt.bo(device, buffer_size, xrt.bo.flags.host_only, kernel.group_id(6))
        
        print(f"Allocated 2 input and 2 output buffers")
        print()
        ...
        for iteration in range(num_iterations):
            # Write input data to both buffers
            bo_in0.write(input_bf16[0], 0)
            bo_in0.sync(xrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE)
            bo_in1.write(input_bf16[1], 0)
            bo_in1.sync(xrt.xclBOSyncDirection.XCL_BO_SYNC_BO_TO_DEVICE)

            # Execute kernel with all 4 buffers
            start = time.perf_counter()
            run = kernel(opcode, bo_instr, len(insts),
                        bo_in0, bo_in1, bo_out0, bo_out1)
            run.wait()
            end = time.perf_counter()
            times.append(end - start)

            # Read output from both buffers
            bo_out0.sync(xrt.xclBOSyncDirection.XCL_BO_SYNC_BO_FROM_DEVICE)
            bo_out1.sync(xrt.xclBOSyncDirection.XCL_BO_SYNC_BO_FROM_DEVICE)
            output0 = bf16_to_float(bo_out0.read(buffer_size, 0).tobytes())
            output1 = bf16_to_float(bo_out1.read(buffer_size, 0).tobytes())
        ...
        print(f"  Average time (2 frames): {avg_time:.3f} ms")
        ...
        batched_time = 5.4   # ms for 4 frames batched
        
        print("Performance Comparison:")
        print(f"  Single kernel: {single_time:.3f} ms per frame")
        print(f"  Sequential (2x single): {single_time * 2:.3f} ms expected")
        print(f"  Parallel (2 tiles): {avg_time:.3f} ms total ({per_frame_time:.3f} ms/frame)")
        print()
        
        expected_sequential = single_time * 2
        if avg_time < expected_sequential:
            speedup = expected_sequential / avg_time
            print(f"  Parallel SPEEDUP: {speedup:.2f}x vs sequential")
        else:
            print(f"  Note: Parallel slower than expected")
        print()

        # Verify accuracy for both frames
        print("Step 9: Verifying accuracy for both frames...")
        outputs = [output0, output1]
        all_pass = True
        
        for i in range(num_frames):
            max_error = np.max(np.abs(outputs[i] - expected[i]))
            correlation = np.corrcoef(outputs[i], expected[i])[0, 1]

//===- softmax_bf16.mlir ----------------------------------------*- MLIR -*-===//
//
// BF16 Softmax Activation for Whisper Encoder - XDNA1 Phoenix NPU
// Based on working GELU and attention patterns
//
// Softmax(x) = exp(x - max(x)) / sum(exp(x - max(x)))
// Using scalar BF16 implementation for numerical stability
//
// Input: [1024] bfloat16 values (typical attention dimension)
...
module @softmax_bf16_npu {
    aie.device(npu1) {
        ...
        // Takes 1024-element BF16 buffer (as raw bytes), applies softmax
        // C++ signature: void softmax_bf16(bfloat16* input, bfloat16* output, int32_t input_size)
        ...
        %core02 = aie.core(%tile02) {
            ...
            scf.for %iter = %c0 to %c_max step %c1 {
                ...
                // Call Softmax kernel: Softmax(input) -> output
                // Scalar BF16 implementation for numerical stability

...
class NPUMelProcessor:
    ...
    def __init__(
        ...
        Args:
            xclbin_path: Path to sign-fixed xclbin file (default: production_kernels/mel_signfix_production.xclbin)
            insts_path: Path to sign-fixed instruction sequence (default: production_kernels/insts_signfix_production.bin)
        ...
        if xclbin_path is None:
            from pathlib import Path
            xclbin_path = str(Path(__file__).parent / "npu_optimization" / "mel_kernels" / "production_kernels" / "mel_signfix_production.xclbin")
        if insts_path is None:
            from pathlib import Path
            insts_path = str(Path(__file__).parent / "npu_optimization" / "mel_kernels" / "production_kernels" / "insts_signfix_production.bin")
